/**
 * Claude 驱动的内部 Agent（M4b）：实现与启发式 Agent 完全相同的内部契约
 * （AgentSideConnection：onPrompt + sessionUpdate + requestPermission + invokeCapability），
 * 但 onPrompt 内部经官方 ACP 驱动 claude-agent-acp 产出 scene.spec.json，再交给 Host 的
 * scene.html 能力出物。因此前端 / 内部协议 / bridge / 能力链路全部不变，仅 Agent 实现替换。
 *
...
import { ClaudeAcpSession, type ClaudeAcpOptions } from './acp-client.js';
import { buildClaudePrompt, buildRetryPrompt, SCENE_SPEC_FILENAME } from './prompt.js';

export interface ClaudeAgentOptions extends ClaudeAcpOptions {
...
export function createClaudeAgent(
  ...
  return new AgentSideConnection(transport, {
    onNewSession(_p: NewSessionParams): NewSessionResult {
      return { sessionId: `claude_${++sessionSeq}` };
    },
    ...
    onPrompt(p: PromptParams, conn: AgentSideConnection): Promise<PromptResult> {
      return run(p, conn, cancelled, options);
    },
  });
}
...
async function run(
  ...
  log(`收到工作流「${graph.name}」：${graph.nodes.length} 节点 / ${graph.edges.length} 连线（Claude）`);

  // 源节点 UI 反馈
  for (const node of graph.nodes.filter((n) => n.kind === 'source')) {
    conn.sessionUpdate(sessionId, { type: 'node-status', nodeId: node.id, status: 'done', message: node.understanding?.ready ? '已理解' : '已登记' });
  }

  const { text, context } = buildClaudePrompt(prompt);
  ...
  const baseDir = prompt.workingDir && isAbsolute(prompt.workingDir)
    ? prompt.workingDir
    : resolve(process.cwd(), prompt.workingDir || '.');
  const cwd = join(baseDir, '.dsweave', 'claude', `${sessionId}_${Date.now()}`);
  mkdirSync(cwd, { recursive: true });
  const specPath = join(cwd, SCENE_SPEC_FILENAME);

  const session = new ClaudeAcpSession(options);
  const toolId = `claude_${outputNodeId}`;
  let lineBuf = '';
  ...
  try {
    ...
    const handlers = {
      ...
      onToolCall: (info: { id: string; title: string; status: string }) =>
        conn.sessionUpdate(sessionId, {
          ...
          id: info.id,
          title: info.title,
          state: info.status === 'completed' ? 'done' : info.status === 'failed' ? 'error' : 'running',
          nodeId: outputNodeId,
        }),
      onPermission: (summary: string) =>
        conn.requestPermission(sessionId, `Claude 请求：${summary}`, ['允许', '拒绝']),
    };
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (outcome.stopReason !== 'end_turn') {
        log(`Claude turn 异常结束：${outcome.stopReason}`, 'warn');
      }
    ...
    if (!spec) {
      conn.sessionUpdate(sessionId, { type: 'node-status', nodeId: outputNodeId, status: 'error', message: 'SceneSpec 未产出/非法' });
    ...
    // 交给 Host 能力出物
    conn.sessionUpdate(sessionId, { type: 'tool-call', id: toolId, title: capability, state: 'running', nodeId: outputNodeId });
    const input: SceneHtmlInput = { spec };
    const res = await conn.invokeCapability({ sessionId, capability, outputNodeId, input });
    conn.sessionUpdate(sessionId, { type: 'tool-call', id: toolId, title: capability, state: 'done', nodeId: outputNodeId });
    conn.sessionUpdate(sessionId, { type: 'artifact', uri: res.artifact.uri, mime: res.artifact.mime, fromNodeId: outputNodeId });
    log(res.cached ? `产物命中缓存：${res.artifact.uri}` : `产物已生成：${res.artifact.uri}（${res.artifact.bytes} 字节）`);
    conn.sessionUpdate(sessionId, { type: 'node-status', nodeId: outputNodeId, status: 'done', message: res.cached ? '产物已就绪（缓存）' : '产物已生成' });

    return finish(conn, sessionId, 'end_turn', session, cwd);
  } catch (err) {
    log(`Claude Agent 失败：${errMessage(err)}`, 'error');
    conn.sessionUpdate(sessionId, { type: 'node-status', nodeId: outputNodeId, status: 'error', message: '产出失败' });
    return finish(conn, sessionId, 'error', session, cwd);
  }
}
